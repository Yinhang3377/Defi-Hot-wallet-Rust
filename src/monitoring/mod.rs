...
impl WalletMetrics {
    pub fn new() -> Result<Self> {
        ...
        // Transaction metrics
        let transactions_sent = Counter::new(
        ...
        let multisig_operations = Counter::new(
            "multisig_operations_total",
            "Total number of multisig operations",
        )?;
        ...
        // Network metrics
        let blockchain_calls = Counter::new(
            "blockchain_calls_total",
            "Total number of blockchain API calls",
        )?;
        let blockchain_errors = Counter::new(
            "blockchain_errors_total",
            "Total number of blockchain API errors",
        )?;
    ...
    pub fn record_transaction_sent(&self, value: f64, fee: f64) {
        ...
        info!(
            "📊 Recorded successful transaction: value={}, fee={}",
            value, fee
        );
    }
...
impl SecurityMonitor {
    ...
    pub async fn check_suspicious_activity(&self, ip: &str, wallet_id: Option<&str>) -> bool {
        ...
        if let Some(wallet_id) = wallet_id {
            ...
            if suspicious_txs >= 3 {
                warn!(
                    "🚨 Detected suspicious transaction activity for wallet {}",
                    wallet_id
                );
                return true;
            }
