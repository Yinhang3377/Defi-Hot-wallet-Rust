...
impl WalletMetrics {
    pub fn new() -> Result<Self> {
        ...
        // Wallet metrics
        let wallets_created = Counter::new("wallets_created_total", "Total number of wallets created")?;
        let wallets_accessed = Counter::new("wallets_accessed_total", "Total number of wallet accesses")?;
        let wallets_deleted = Counter::new("wallets_deleted_total", "Total number of wallets deleted")?;
        
        // Transaction metrics
        let transactions_sent = Counter::new("transactions_sent_total", "Total number of transactions sent")?;
        let transactions_failed = Counter::new("transactions_failed_total", "Total number of failed transactions")?;
        let transaction_value = Histogram::new("transaction_value", "Transaction values in native tokens")?;
        let transaction_fees = Histogram::new("transaction_fees", "Transaction fees in native tokens")?;
        
        // Security metrics
        let login_attempts = Counter::new("login_attempts_total", "Total number of login attempts")?;
        let failed_logins = Counter::new("failed_logins_total", "Total number of failed logins")?;
        let quantum_encryptions = Counter::new("quantum_encryptions_total", "Total number of quantum encryptions performed")?;
        let multisig_operations = Counter::new("multisig_operations_total", "Total number of multisig operations")?;
        
        // Performance metrics
        let active_connections = Gauge::new("active_connections", "Number of active connections")?;
        let response_time = Histogram::new("response_time_seconds", "Response time in seconds")?;
        let database_operations = Histogram::new("database_operations_seconds", "Database operation time in seconds")?;
        
        // Network metrics
        let blockchain_calls = Counter::new("blockchain_calls_total", "Total number of blockchain API calls")?;
        let blockchain_errors = Counter::new("blockchain_errors_total", "Total number of blockchain API errors")?;
        let network_latency = Histogram::new("network_latency_seconds", "Network latency in seconds")?;
...
impl SecurityMonitor {
    pub fn new(metrics: Arc<WalletMetrics>) -> Self {
        ...
        Self {
            metrics,
            suspicious_activity: Arc::new(Mutex::new(Vec::new())),
        }
    }
    ...
    pub async fn check_suspicious_activity(&self, ip: &str, wallet_id: Option<&str>) -> bool {
        ...
        if failed_logins >= 5 {
            warn!("🚨 Detected suspicious activity: {} failed logins from IP {}", failed_logins, ip);
            return true;
        }
        
        // Check for suspicious transactions if wallet_id is provided
        if let Some(wallet_id) = wallet_id {
            let suspicious_txs = events.iter()
                .filter(|e| {
    ...
    async fn send_critical_alert(&self, event: &SecurityEvent) {
        ...
        error!("Alert details: IP={:?}, Wallet={:?}, Time={}", 
...
static SECURITY_MONITOR: once_cell::sync::OnceCell<Arc<SecurityMonitor>> = once_cell::sync::OnceCell::new();
...
pub async fn init_metrics() -> Result<()> {
    ...
    SECURITY_MONITOR.set(security_monitor).map_err(|_| anyhow::anyhow!("Security monitor already initialized"))?;
