...
impl WalletStorage {
    pub async fn new() -> Result<Self> {
        Self::new_with_url("sqlite:./wallet.db").await
    }
    ...
    async fn initialize_schema(&self) -> Result<()> {
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_transactions_wallet_id ON transactions (wallet_id)")
            .execute(&self.pool)
            .await?;
        
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_transactions_tx_hash ON transactions (tx_hash)")
            .execute(&self.pool)
            .await?;
        
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_audit_logs_wallet_id ON audit_logs (wallet_id)")
            .execute(&self.pool)
            .await?;
    ...
    pub async fn store_wallet(&self, name: &str, encrypted_data: &[u8]) -> Result<()> {
        ...
        // Log the action
        self.log_action(&wallet_id, "wallet_created", &format!("Wallet '{}' created", name), None, None).await?;
    ...
    pub async fn load_wallet(&self, name: &str) -> Result<Vec<u8>> {
        ...
        let row = sqlx::query(
            "SELECT id, encrypted_data FROM wallets WHERE name = ?1"
        )
        .bind(name)
        ...
        match row {
            Some(row) => {
                ...
                // Log the action
                self.log_action(&wallet_id, "wallet_accessed", &format!("Wallet '{}' accessed", name), None, None).await?;
    ...
    pub async fn delete_wallet(&self, name: &str) -> Result<()> {
        ...
        let wallet_id = match row {
            ...
            None => return Err(anyhow::anyhow!("Wallet not found: {}", name)),
        };
        ...
        // Log the action
        self.log_action(&wallet_id, "wallet_deleted", &format!("Wallet '{}' deleted", name), None, None).await?;
