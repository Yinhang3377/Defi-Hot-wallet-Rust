...
impl SolanaClient {
    ...
    fn validate_solana_address(address: &str) -> bool {
        ...
        // Real Solana addresses are 32-byte base58 encoded strings
...
impl BlockchainClient for SolanaClient {
    async fn get_balance(&self, address: &str) -> Result<String> {
        ...
        if !Self::validate_solana_address(address) {
    ...
    async fn send_transaction(
        ...
        if !Self::validate_solana_address(to_address) {
            return Err(anyhow::anyhow!("Invalid recipient address: {}", to_address));
        }
        
        // Parse amount
        let _amount_f64: f64 = amount.parse()
    ...
    fn validate_address(&self, address: &str) -> Result<bool> {
        Ok(Self::validate_solana_address(address))
    }
...
mod tests {
    ...
    fn test_address_validation() {
        ...
        assert!(SolanaClient::validate_solana_address("11111111111111111111111111111112"));
        
        // Invalid addresses
        assert!(!SolanaClient::validate_solana_address("invalid_address"));
        assert!(!SolanaClient::validate_solana_address("0x742d35Cc6635C0532925a3b8D400e8B78fFe4860")); // Ethereum format
    }
    ...
    async fn test_solana_client() {
        ...
        assert!(client.validate_address("11111111111111111111111111111112").unwrap());
