...
impl HSMManager {
    ...
    pub async fn initialize(&mut self, config: HSMConfig) -> Result<()> {
        ...
        if self.config.enabled {
            ...
            info!(
                "🛡️ Memory isolation enabled: {}",
                self.config.isolation_enabled
            );
        } else {
    ...
    pub async fn write_secure_memory(&self, region_id: u64, data: &[u8]) -> Result<()> {
        debug!(
            "Writing {} bytes to secure memory region {}",
            data.len(),
            region_id
        );
    ...
    pub async fn secure_key_generation(&self, key_type: &str, key_size: usize) -> Result<u64> {
        info!(
            "🔑 Generating secure key: {} (size: {} bytes)",
            key_type, key_size
        );
    ...
    pub async fn get_memory_stats(&self) -> Result<HSMMemoryStats> {
        ...
        Ok(HSMMemoryStats {
            ...
            average_region_size: if total_regions > 0 {
                total_memory / total_regions
            } else {
                0
            },
        })
