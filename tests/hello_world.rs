fn approx_eq_str(a: &str, b: &str) -> bool {
    ...
    if aa.is_nan() || bb.is_nan() { return false; }
    let diff = (aa - bb).abs();
...
fn test_get_transaction_status_confirmed() {
    let transaction_status: Result<&str, &str> = Err("Failed to get transaction receipt: missing field `transactionIndex`"); // Example error
...
fn test_get_transaction_status_pending() {
    let transaction_status: Result<&str, &str> = Err("Failed to get transaction receipt: missing field `transactionHash`"); // Example error
    assert!(transaction_status.is_err());
}
